  ...
  if (!task) {
    return (
      ...
      >
        ...
        <div style={{ font: '600 14.5px sans-serif', color: '#94A3B8' }}>Chọn một công việc để xem chi tiết</div>
      </div>
    );
  }
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '6px', marginBottom: '6px' }}>
        <button
          onClick={() => onEdit(task)}
          title="Chỉnh sửa"
          style={{ background: '#F1F5F9', border: 'none', borderRadius: '8px', padding: '8px', display: 'flex', cursor: 'pointer' }}
        >
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', marginBottom: '16px' }}>
        ...
        <div
          style={{
            font: "700 20px/1.35 'Space Grotesk',sans-serif",
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
        <span style={{ font: '600 13px sans-serif', color: cat.color, background: cat.tint, padding: '5px 12px', borderRadius: '8px' }}>
          {cat.label}
        </span>
        <span style={{ font: '600 13px sans-serif', color: pr.color, background: pr.bg, padding: '5px 12px', borderRadius: '8px' }}>
          Ưu tiên {pr.label}
        </span>
      </div>
      ...
      {/* Description */}
      {task.desc && (
        <div style={{ font: '500 15px/1.6 sans-serif', color: '#334155', marginBottom: '16px', wordBreak: 'break-word' }}>
          {task.desc}
        </div>
      )}
